...
class Book(models.Model):
    ...
    cover_type = CharField(max_length=20, blank=True,
                           verbose_name='Тип обложки')
    # book size
    book_format = CharField(max_length=30, blank=True, verbose_name='Формат')
...
class Order(models.Model):
    ...
    book_id = ManyToManyField(Book, verbose_name='Книги',
                              related_name='orders_by_book')
...
class OrderDetails(models.Model):
    ...
    order_id = ForeignKey(Order, on_delete=CASCADE, verbose_name='Заказ',
                          related_name='books')
    book_id = ForeignKey(Book, on_delete=SET_NULL, null=True,
                         verbose_name='Книга', related_name='orders')
    quantity = IntegerField(default=1, verbose_name='Количество',
                            validators=(MinValueValidator(0),
                                        MaxValueValidator(50)))
    cost = IntegerField(verbose_name='Стоимость')
